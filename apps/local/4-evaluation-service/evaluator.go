...
func (a *App) fetchFlag(flagName string) (*Flag, error) {
	url, err := buildServiceURL(a.FlagServiceURL, "flags", flagName)
	if err != nil {
		return nil, fmt.Errorf("url inválida do flag-service: %w", err)
	}
	...
	// #nosec G704 -- Base URL é de configuração controlada e o path é escapado.
	...
	// #nosec G704 -- Chamada restrita a serviços internos configurados.
...
func (a *App) fetchRule(flagName string) (*TargetingRule, error) {
	url, err := buildServiceURL(a.TargetingServiceURL, "rules", flagName)
	if err != nil {
		return nil, fmt.Errorf("url inválida do targeting-service: %w", err)
	}
...
func buildServiceURL(baseURL, resource, flagName string) (string, error) {
	...
	if (parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.Host == "" {
		return "", fmt.Errorf("base URL deve ter scheme http/https e host")
	}

	parsed.Path = strings.TrimRight(parsed.Path, "/") + "/" + resource + "/" + url.PathEscape(flagName)
	parsed.RawQuery = ""
	parsed.Fragment = ""
